...
def findPlotValues(plot):
  fluorescence = np.zeros((plot.shape))
  timeIndex = 0
  while (timeIndex<plot.shape[1]):
    ftszIndex = 0
    amplitude = 0
    while ((ftszIndex<plot.shape[1]) and (amplitude<plot.shape[0])):
...
fileName = 'chen2005_fig4b.png'
imOriginal = plt.imread(fileName)
im = imOriginal[:, :, 0]
im = np.multiply(im, -1)
...
time = np.linspace(0, totalTime, num=fluorescence.shape[1])
...
startFtsz = np.array([1])
concMonomer = np.zeros((dataPoints, fluorescence.shape[1]))
plt.figure()
for ii in range(dataPoints):
  concMonomer[ii,:] = lightSignalToConcMonomer(fluorescence[ii,:], startFtsz[ii], finalMix)
  plt.plot(time, concMonomer[ii,:])
plt.show()
result = np.vstack((concMonomer, time))
result = np.rot90(result, k=3)

np.savetxt(fileName[:-4]+'.csv', result, delimiter=",", fmt='%f')
